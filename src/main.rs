...
struct Resources {
    ...
    enemey_small_texture: Texture2D,
    theme_music: Sound,
...
impl Resources {
    ...
    let ship_texture = load_texture("ship.png").await.expect("Couldn't load file");
    ship_texture.set_filter(FilterMode::Nearest);
    let bullet_texture = load_texture("laser-bolts.png").await.expect("Couldn't load file");
    bullet_texture.set_filter(FilterMode::Nearest);
    let explosion_texture = load_texture("explosion.png").await.expect("Couldn't load file");
    explosion_texture.set_filter(FilterMode::Nearest);
    let enemey_small_texture = load_texture("enemy-small.png").await.expect("Couldn't load file");
    enemey_small_texture.set_filter(FilterMode::Nearest);
        ...
        match game_state {
            GameState::MainMenu => {
                root_ui().window(
                    hash!(), 
                    vec2(
                        screen_width() / 2.0 - window_size.x / 4.0,
            ...
            GameState::Playing => {
                ...
                for square in &squares {
                    ...
                    // );
                    draw_texture_ex(
                        &resources.enemey_small_texture,
                        square.x - square.size / 2.0,
                        square.y - square.size / 2.0,
                        WHITE,
                        DrawTextureParams {
            ...
            GameState::GameOver => {
                ...
                if score > prev_highscore {
                    text = "Congratulation!!! You have the Highscore";
                } else {
                    text = "GAME OVER!";
                }
